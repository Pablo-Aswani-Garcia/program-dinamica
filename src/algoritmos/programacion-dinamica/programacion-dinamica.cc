 ...
 * @brief Definición de la clase ProgramacionDinamica
 * @see
 */
...
/**
 * Implementación del algoritmo de programación dinámica para resolver el
 *problema del TSP.
 * @param transiciones Matriz de distancias entre ciudades.
 * @param ciudades Vector con los nombres de las ciudades.
 * @return std::pair<std::vector<std::string>, int> Un par con la mejor ruta y
 *su coste.
 **/
std::pair<std::vector<std::string>, int> ProgramacionDinamica::resolver(
  ...
  std::vector<std::vector<int>> tabla_memoizacion(
      1 << num_ciudades, std::vector<int>(num_ciudades, INT_MAX));

  tabla_memoizacion[1][0] =
      0;  // Estado inicial: solo la ciudad 0 visitada, costo 0
  ...
  for (int mask = 1; mask < (1 << num_ciudades); ++mask) {
    ...
    for (int ciudad = 0; ciudad < num_ciudades; ++ciudad) {
      ...
      if (mask & (1 << ciudad)) {
        for (int ciudad_destino = 0; ciudad_destino < num_ciudades;
             ++ciudad_destino) {
          // Si la ciudad destino no ha sido visitada
          if (!(mask & (1 << ciudad_destino))) {
            int nueva_mask = mask | (1 << ciudad_destino);
            if (tabla_memoizacion[mask][ciudad] !=
                INT_MAX) {  // Evitar suma con INT_MAX
              tabla_memoizacion[nueva_mask][ciudad_destino] =
                  std::min(tabla_memoizacion[nueva_mask][ciudad_destino],
                           tabla_memoizacion[mask][ciudad] +
                               transiciones[ciudad][ciudad_destino]);
            }
          }
        }
      }
  ...
  for (int i = num_ciudades - 1; i >= 0; --i) {
    ...
    for (int j = 0; j < num_ciudades; ++j) {
      // si j esta en mask y el costo de llegar a pos desde j es igual al costo
      // de llegar a pos desde j
      if ((mask & (1 << j)) && tabla_memoizacion[mask][pos] != INT_MAX &&
          tabla_memoizacion[mask][pos] ==
              tabla_memoizacion[mask ^ (1 << pos)][j] + transiciones[j][pos]) {
        next_pos = j;
        break;
      }
    }
